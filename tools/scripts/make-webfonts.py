...
def getVarAxes(ttf):
    ...
    if "STAT" in ttf and hasattr(ttf["STAT"], "table"):
        axes["order"] = [
            a.AxisTag
            for a in sorted(
                ttf["STAT"].table.DesignAxisRecord.Axis, key=lambda a: a.AxisOrdering
            )
        ]
    ...
    if "fvar" in ttf:
        for axis in ttf["fvar"].axes:
            axes[axis.axisTag] = {
                "name": getName(
                    ttf, axis.nameID if hasattr(axis, "nameID") else axis.axisNameID
                ),
                "min": axis.minValue,
                "max": axis.maxValue,
...
with open(os.path.join(outdir, "axes.json"), "wb") as axesfile:
