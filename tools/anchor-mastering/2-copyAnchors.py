...
selectedGlyphsAccentsUC = [
    "circumflexacutecomb.case",
    "brevehookabovecomb.case",
    "circumflextildecomb.case",
    "brevegravecomb.case",
    "circumflexhookabovecomb.case",
    "breveacutecomb.case",
    "circumflexgravecomb.case",
    "brevetildecomb.case",
    "breve.cyr.case",
    "gravecomb.case",
    "acutecomb.case",
    ...
    "acutecombstack.case",
    "brevecombstack.case",
    "circumflexcombstack.case",
    "dieresiscombstack.case",
    "dotaccentcombstack.case",
    "gravecombstack.case",
    "hookabovecombstack.case",
    "macroncombstack.case",
    "tildecombstack.case",
]

selectedGlyphsAccentslc = [
    "brevecomb",
    "acutecomb",
    "tildecomb",
    "dblgravecomb",
        ...
        for anchor in srcGlyph.anchors:
            ...
            if anchor.name == "top":
                if "comb" in dstGlyph.name:
                    dstGlyph.appendAnchor(
                        anchor.name,
                        ((dstGlyph.width) / 2, abs(dstFont[dstGlyph.name].topMargin)),
                    )
                else:
                    dstGlyph.appendAnchor(
                        anchor.name, ((dstGlyph.width) / 2, yPositioning)
                    )
                ...
                if "comb" in dstGlyph.name:
                    dstGlyph.appendAnchor(
                        anchor.name,
                        ((dstGlyph.width) / 2, dstFont[dstGlyph.name].bottomMargin),
                    )
                else:
                ...
                dstGlyph.appendAnchor(
                    anchor.name,
                    ((dstGlyph.width - dstGlyph.rightMargin - 10), anchor.y),
                )
                ...
                dstGlyph.appendAnchor(
                    anchor.name, ((dstGlyph.width - dstGlyph.rightMargin), yPositioning)
                )

            elif anchor.name == "_topright":
                dstGlyph.appendAnchor(
                    anchor.name, ((dstGlyph.leftMargin / 2), yPositioning)
                )
